$(document).ready(function (e) {
    ...
    $('#thembophan').on('click', function (e) {
        ...
        if (name.length == 0) {
            ...
            checkname = false;
            $('#BoPhanValidateResul').text("Tên bộ phận không hợp lệ! Vui lòng kiểm tra lại.").show();
        } else if (name.length > 50) {
            checkname = false;
            $('#BoPhanValidateResul').text("Tên bộ phận chỉ tối đa 50 ký tự! Vui lòng kiểm tra lại.").show();
        }

        if (mota.length > 200) {
            checkmota = false;
            $('#MoTaBoPhanValidateResul').text("Mô tả bộ phận chỉ tối đa 200 ký tự! Vui lòng kiểm tra lại.").show();
        }
                ...
                if (ketqua === "DANHSACH") {
                    ...
                    var SweetAlert2Demo = function () {
                        var initDemos = function () {
                            swal("Thành Công!", "Tuyệt quá! Một bộ phận đã được thêm vào danh sách.", {
                                icon: "success",
                                buttons: {
                                    confirm: {
